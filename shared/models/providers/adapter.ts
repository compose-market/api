...
import { getEmbeddingModel, getLanguageModel } from "../registry.js";
...
async function invokeVideo(request: UnifiedRequest, target: AdapterTarget): Promise<UnifiedOutput> {
  ...
  if (["aiml", "openai", "google", "vertex"].includes(target.provider)) {
...
async function invokeAudio(request: UnifiedRequest, target: AdapterTarget): Promise<UnifiedOutput> {
  ...
  const output = await withRetry(async () => {
    switch (target.provider) {
      case "openai":
        return openaiGenerateSpeech(target.modelId, prompt, {
          voice: request.audioOptions?.voice as any,
          speed: request.audioOptions?.speed,
          responseFormat: request.audioOptions?.responseFormat as any,
        });
      case "google":
        return googleGenerateSpeech(target.modelId, prompt);
      case "vertex":
        return (await generateVertexSpeech(target.modelId, prompt, { voice: request.audioOptions?.voice })).buffer;
      case "huggingface": {
        const payload: HFInferenceInput = {
          modelId: target.modelId,
          task: "text-to-speech",
  ...
  const format = request.audioOptions?.responseFormat || "mp3";
  ...
  return {
    modality: "audio",
    media: {
      mimeType: contentTypes[format] || "audio/mpeg",
