...
function getModelsBasePath(): string {
    // Lambda environment note:
    // - esbuild CJS output: __dirname points to /var/task/dist/
    // - The data folder is at /var/task/dist/data/
    // - process.cwd() is /var/task in Lambda

    const candidatePaths: string[] = [];

    // 1. Try __dirname first (works with esbuild CJS output - points to dist/)
...
export function isValidModelId(modelId: string): boolean {
    return getModelCard(modelId) !== null;
}
...
export function formatModelName(modelId: string): string {
    ...
    return modelId
        .replace(/^(models\/|meta-llama\/|mistralai\/|google\/|openai\/|anthropic\/)/, "")
        .replace(/-/g, " ")
